...
class CustomGroup {
  ...
  getWidthAndHeight(nodes){
    let len = nodes.length -1;
    let firstY = graph.findById(nodes[0]).getBBox();
    let lastY = graph.findById(nodes[len]).getBBox();

    return {
      width: firstY.width,
      height: lastY.y + lastY.height - firstY.y
    };
  }
  ...
  create(groupId, nodes, type = 'circle', zIndex = 0, updateDataModel = false) {
    ...
    // 重要 将组放进节点图层
    const customGroup = graph.get('nodeGroup');
    ...
    if (type === 'circle') {
      ...
      let {width,height} = this.getWidthAndHeight(nodes);
      ...
      // 更新群组及属性样式 todo 拖拽优化
      this.setDeletageGroupByStyle(groupId, nodeGroup,{ width, height, x, y, btnOffset: maxX - 3 });
    }
